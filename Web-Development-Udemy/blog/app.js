...
app.get("/", function(req, res){
  res.render("home", {homeContent : homeStartingContent, listBlog : listDiary});
});
...
app.post("/compose", function(req, res){
  ...
  listDiary.push(newBlog);
  console.log(listDiary);
  res.redirect("/");
})

app.get("/post/:postname", function(req, res){
  const curTilte = req.params.postname;
  let curContent = "";
  listDiary.forEach(blog => {
      if(blog.title.localeCompare(curTilte) == 0){
        curContent = blog.content;
      }
      else
      {
  ...
  res.render("post", {title: curTilte, content: curContent});
})
